...
async function addUsage() {
  ...
  try {
    const res = await fetch(`${API_BASE}/usageLog/add`, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        applianceId: Number(applianceId.value),
...
/* Load usage logs when page opens */
loadUsageLogs();
</script>
...
<template>
  <div class="container">
    ...
    <!-- EXISTING USAGE LOGS -->
    <div class="box" v-if="usageLogs.length > 0">
      <h2>Existing Usage Logs</h2>

      <div v-for="log in usageLogs" :key="log.usageLogId">
        <p><strong>ID:</strong> {{ log.usageLogId }}</p>
        <p><strong>Hours Used:</strong> {{ log.hoursUsed }}</p>
        <p><strong>Date:</strong> {{ log.date }}</p>
        <p><strong>kWh:</strong> {{ log.calculatedKwh }}</p>
        <p><strong>Cost:</strong> €{{ log.calculatedCost }}</p>
        <hr />
      </div>
    </div>
